...
// AdSense slot IDs (replace with your actual AdSense ad unit slot IDs)
const AD_SLOTS = {
  bannerTop: 'YOUR_BANNER_TOP_SLOT', // e.g., '1234567890'
  bannerBottom: 'YOUR_BANNER_BOTTOM_SLOT',
  sidebar: 'YOUR_SIDEBAR_SLOT',
  native: 'YOUR_NATIVE_SLOT',
  floating: 'YOUR_FLOATING_SLOT',
};
...
export default function Home() {
  ...
  // Insert ads into product grid at strategic positions
  const getDisplayedProductsWithAds = () => {
    const result = [];
    let adCounter = 0;
    ...
    displayedProducts.forEach((product, index) => {
      ...
      // Insert native ad every 8 products
      if ((index + 1) % 8 === 0 && shouldShowAd(`native-ad-${adCounter}`)) {
        result.push({
          id: `native-ad-${adCounter}`,
          isAd: true,
          adType: 'native',
          adSlot: AD_SLOTS.native,
          cycleKey: `ad-native-${adCounter}-${index}`,
        });
        adCounter++;
      }

      // Insert sidebar ad every 12 products
      if ((index + 1) % 12 === 0 && shouldShowAd(`sidebar-ad-${Math.floor(index / 12)}`)) {
        result.push({
          id: `sidebar-ad-${Math.floor(index / 12)}`,
          isAd: true,
          adType: 'sidebar',
          adSlot: AD_SLOTS.sidebar,
          cycleKey: `ad-sidebar-${Math.floor(index / 12)}-${index}`,
        });
      }
    });
  ...
  const loadMoreProducts = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (newProducts.length < ITEMS_PER_PAGE) {
        ...
        const nextCycleProducts = sortOption === 'random' || sortOption === '' ? shuffleArray(filteredProducts) : filteredProducts;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      ...
      {shouldShowAd('banner-top') && (
        <BannerAd adSlot={AD_SLOTS.bannerTop} position="top" onClose={() => closeAd('banner-top')} />
      )}
        ...
        {filteredProducts.length === 0 ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {displayedProductsWithAds.map((item, index) => (
                <div
                  key={item.cycleKey}
                  ref={index === displayedProductsWithAds.length - 1 ? lastProductElementRef : null}
                >
                  {item.isAd ? (
                    item.adType === 'native' ? (
                      <NativeAd adSlot={item.adSlot} />
                    ) : item.adType === 'sidebar' ? (
      ...
      {shouldShowAd('banner-bottom') && (
        <BannerAd adSlot={AD_SLOTS.bannerBottom} position="bottom" onClose={() => closeAd('banner-bottom')} />
      )}

      <Footer />

      {shouldShowAd('floating-ad') && (
        <FloatingAd adSlot={AD_SLOTS.floating} onClose={() => closeAd('floating-ad')} />
      )}
